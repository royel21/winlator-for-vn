...
public class ForegroundService extends Service {
    ...
    private final androidx.lifecycle.DefaultLifecycleObserver appLifecycleObserver = new androidx.lifecycle.DefaultLifecycleObserver() {
        @Override
    ...
    private void acquireWakeLock() {
        ...
        if (prefs == null)
            return;
        if (!prefs.getBoolean(PREF_USE_WAKELOCK, false))
            return;
        if (!wakeLock.isHeld()) {
            wakeLock.acquire();
